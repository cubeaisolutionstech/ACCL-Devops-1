...
const EROPWAnalysis = ({ 
  ...
  const [activeSubTab, setActiveSubTab] = useState('tonnage');
  ...
  useEffect(() => {
    if ((eroPwData.mt_data || eroPwData.value_data) && autoExportEnabled && !hasCheckedData) {
  ...
  }, [eroPwData, autoExportEnabled, autoGenerateEroPwReport, hasCheckedData]);
    ...
    try {
      ...
      if (result.success) {
        ...
        if (mtRows > 0 && valueRows > 0) {
          ...
          addMessage('⚠️ ERO-PW analysis completed but no data was generated. Please check your input files.', 'warning');
        }
      } else {
  ...
  // Export function with latest only storage
  const exportEroPwReport = async () => {
    const hasMtData = eroPwData.mt_data && eroPwData.mt_data.length > 0;
    const hasValueData = eroPwData.value_data && eroPwData.value_data.length > 0;
    
    if (!hasMtData && !hasValueData) {
      addMessage('No ERO-PW data available to export. Please ensure MT or Value analysis completed successfully.', 'error');
      return;
    }

    console.log('📤 Exporting ERO-PW tables');
    setLoading(true);
    
    try {
      const timestamp = new Date().toISOString().slice(0, 19).replace(/[:-]/g, '');
      const fileName = `eropw_combined_manual_${fiscalInfo.current_year || 'report'}_${timestamp}.xlsx`;
      ...
      if (result.success) {
        ...
        addMessage(`📁 ERO-PW analysis exported successfully`, 'success');
      } else {
  ...
  const DataTable = ({ data, columns, title, dataType }) => {
        ...
        <div className="table-section">
          ...
          {eroPwData.mt_data ? (
            ...
            <div className="table-empty-state">
              <Package size={48} />
              <h4>No ERO-PW Tonnage Data Available</h4>
              <p>
                {!canProcess() ? 
        ...
        <div className="table-section">
          ...
          {eroPwData.value_data ? (
            <DataTable
              ...
              columns={eroPwData.columns.value}
          ...
          {(eroPwData.mt_data || eroPwData.value_data) && (
            <div className="export-merged-section">
              <div className="export-actions">
                <button
                  onClick={exportEroPwReport}
                  className="btn btn-primary"
                  disabled={loading}
                  title="Download ERO-PW report"
                >
                  <Download size={16} />
  ...
  return (
    <div className="eropw-analysis-section">
      ...
      <nav className="sub-tab-navigation">
        <button
          className={`sub-tab ${activeSubTab === 'tonnage' ? 'active' : ''}`}
          onClick={() => setActiveSubTab('tonnage')}
      ...
      <div className="sub-tab-content">
        {activeSubTab === 'tonnage' && (
          <div className="tonnage-analysis">
            {eroPwData.mt_data ? (
              <DataTable
                data={eroPwData.mt_data}
                columns={eroPwData.columns.mt}
