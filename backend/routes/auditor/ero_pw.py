...
ero_pw_bp = Blueprint('ero_pw', __name__, url_prefix='/api/ero-pw')

class EROPWMergePreviewProcessor:
    """Handles the complete ERO-PW merge preview logic with EXACT column ordering"""
    ...
    def __init__(self):
        ...
        self.exclude_from_sort = ['TOTAL SALES', 'GRAND TOTAL', 'WEST TOTAL', 'NORTH SALES']
    ...
    def calculate_ytd_growth_achievement(self, merged_data):
        """Calculate YTD Growth and Achievement with EXACT column names - UPDATED VERSION with conditional calculations"""
        current_fy = self.fiscal_info['fiscal_year_str']
        last_fy = self.fiscal_info['last_fiscal_year_str']
...
ero_pw_merge_processor = EROPWMergePreviewProcessor()
...
def process_budget_data_product_region(budget_df, group_type='product_region'):
    """Process budget data for ERO-PW analysis"""
...
def process_sales_data(df_sales, fiscal_year_start, months):
    """
    Process current year sales data for ERO-PW analysis
    ...
    try:
        ...
        if 'Actual_Quantity' in df_sales.columns and 'Product_Group' in df_sales.columns and 'Month_Format' in df_sales.columns:
            ...
            # Filter for WEST region if region column exists
            if 'Region' in df_qty.columns:
                df_qty = df_qty[df_qty['Region'].str.strip().str.upper() == 'WEST']
        ...
        if 'Value' in df_sales.columns and 'Product_Group' in df_sales.columns and 'Month_Format' in df_sales.columns:
            ...
            if 'Region' in df_value.columns:
                df_value = df_value[df_value['Region'].str.strip().str.upper() == 'WEST']
...
def build_exact_columns_and_calculate_values(data_df, fiscal_info, analysis_type='mt'):
    ...
    # Calculate YTD columns in exact order and position with conditional calculations
...
def process_ero_pw_analysis():
    ...
    try:
        ...
        # ========== CORRECTED LAST YEAR DATA PROCESSING LOGIC ==========        
        actual_mt_last = {}
        actual_value_last = {}
        ...
        # SECOND ATTEMPT: If first method failed, use SALES PROCESSING LOGIC on LAST YEAR FILE
        if ly_processing_method == "none" and last_year_filepath and last_year_sheet:
            try:
                current_app.logger.info("Attempting SECOND method: Sales processing logic on last year file")
                xls_last_year = pd.ExcelFile(last_year_filepath)
                
                # Check if last year file has multiple sheets, try to process them like sales sheets
                available_sheets = xls_last_year.sheet_names
                sheets_to_process = [last_year_sheet] if last_year_sheet in available_sheets else available_sheets
                
                for sheet_name in sheets_to_process:
                    try:
                        df_last_year_sales = pd.read_excel(xls_last_year, sheet_name=sheet_name, header=0)
                        
                        if isinstance(df_last_year_sales.columns, pd.MultiIndex):
                            df_last_year_sales.columns = ['_'.join(col).strip() for col in df_last_year_sales.columns.values]
                        df_last_year_sales = handle_duplicate_columns(df_last_year_sales)
                        
                        # USE SAME COLUMN DETECTION AS CURRENT YEAR SALES
                        region_col = find_column(df_last_year_sales, ['Region', 'Area', 'Zone'], case_sensitive=False)
                        product_col = find_column(df_last_year_sales, ['Type (Make)', 'Type(Make)', 'Product'], case_sensitive=False)
                        date_col = find_column(df_last_year_sales, ['Date', 'Month Format', 'Month'], case_sensitive=False)
                        qty_col = find_column(df_last_year_sales, ['Actual Quantity', 'Quantity', 'Qty'], case_sensitive=False)
                        value_col = find_column(df_last_year_sales, ['Value', 'Amount', 'Sales Value'], case_sensitive=False)
                        
                        if product_col:
                            # Filter for WEST region
                            if region_col:
                                df_last_year_sales = df_last_year_sales[df_last_year_sales[region_col].str.strip().str.upper() == 'WEST']
                            
                            # Add products to the set
                            unique_products = df_last_year_sales[product_col].dropna().astype(str).str.strip().str.upper()
                            all_products.update(unique_products)
                            
                            # Process actual sales data if we have required columns
                            if date_col and not df_last_year_sales.empty:
                                # USE SAME DATE PROCESSING AS CURRENT YEAR
                                if pd.api.types.is_datetime64_any_dtype(df_last_year_sales[date_col]):
                                    df_last_year_sales['Month'] = pd.to_datetime(df_last_year_sales[date_col]).dt.strftime('%b')
                                else:
                                    month_str = df_last_year_sales[date_col].astype(str).str.strip()
                                    try:
                                        df_last_year_sales['Month'] = pd.to_datetime(month_str, format='%Y-%m', errors='coerce').dt.strftime('%b')
                                    except ValueError:
                                        df_last_year_sales['Month'] = month_str.str[:3]
                                
                                # Process quantity data (SAME LOGIC AS CURRENT YEAR)
                                if qty_col:
                                    df_qty = df_last_year_sales[[product_col, 'Month', qty_col]].copy()
                                    df_qty[qty_col] = pd.to_numeric(df_qty[qty_col], errors='coerce')
                                    df_qty = df_qty.dropna(subset=[qty_col, 'Month'])
                                    df_qty = df_qty[df_qty[qty_col] != 0]
                                ...
                                # Process value data (SAME LOGIC AS CURRENT YEAR)
                                if value_col:
                                    df_value = df_last_year_sales[[product_col, 'Month', value_col]].copy()
                                    df_value[value_col] = pd.to_numeric(df_value[value_col], errors='coerce')
                                    df_value = df_value.dropna(subset=[value_col, 'Month'])
                                    df_value = df_value[df_value[value_col] != 0]
                        ...
                        current_app.logger.warning(f"Error processing last year sheet {sheet_name}: {str(sheet_error)}")
                        continue
                
                # Check if we got meaningful data from the second method
                total_ly_records = len(actual_mt_last) + len(actual_value_last)
                if total_ly_records > 0:
                    ly_processing_method = "sales_logic_on_last_year_file"
                    current_app.logger.info(f"SUCCESS: Second method worked - Got {total_ly_records} LY records from last year file using sales logic")
                else:
        ...
        if mt_cols:
            ...
            for month_col in month_cols:
                if f'{month_col}_MT' in budget_data.columns:
                    result_ero_pw_mt[month_col] = 0.0
                    for _, row in budget_data.iterrows():
                        ...
                        if pd.notna(value):
                            ...
                            if not idx.empty:
                                result_ero_pw_mt.loc[idx, month_col] = value
            ...
            result_ero_pw_mt = remove_specific_unwanted_columns_fixed(result_ero_pw_mt, 'SALES in Tonage')
            
            # Apply EXACT fiscal year column ordering
            result_ero_pw_mt = ero_pw_merge_processor.reorder_columns_exact_fiscal_year(result_ero_pw_mt, 'SALES in Tonage')
            
            mt_result = result_ero_pw_mt.fillna(0).round(2).to_dict('records')
            mt_columns = list(result_ero_pw_mt.columns)
        ...
        if value_cols:
            ...
            for month_col in month_cols:
                if f'{month_col}_Value' in budget_data.columns:
                    result_ero_pw_value[month_col] = 0.0
                    for _, row in budget_data.iterrows():
                        product = row['PRODUCT NAME']
                        ...
                        if pd.notna(value):
                            ...
                            if not idx.empty:
                                result_ero_pw_value.loc[idx, month_col] = value
            ...
            # Remove specific unwanted columns
            result_ero_pw_value = remove_specific_unwanted_columns_fixed(result_ero_pw_value, 'SALES in Value')
            
            # Apply EXACT fiscal year column ordering
            result_ero_pw_value = ero_pw_merge_processor.reorder_columns_exact_fiscal_year(result_ero_pw_value, 'SALES in Value')
            
            value_result = result_ero_pw_value.fillna(0).round(2).to_dict('records')
            value_columns = list(result_ero_pw_value.columns)
            ...
            'data': {
                ...
                'ly_processing_info': {
                    ...
                    'description': {
                        ...
                        'sales_logic_on_last_year_file': 'Used sales processing logic on last year file',
        ...
        current_app.logger.error(f"Error in ERO-PW analysis: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@ero_pw_bp.route('/export-combined-ero-pw-excel', methods=['POST'])
def export_combined_ero_pw_excel():
    """Export both MT and Value data to Excel with enhanced formatting matching TS-PW style"""
    try:
        data = request.json
        ...
        filename = f"ero_pw_combined_analysis_west_{fiscal_year}_{timestamp}.xlsx"
        
        output = BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            workbook = writer.book
            
            # Enhanced formats matching TS-PW style exactly
        ...
        current_app.logger.error(f"Error in enhanced ERO-PW export: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
